...
def backward():
    ...
    GPIO.output(pin2, False)
    GPIO.output(pin3, True)
    GPIO.output(pin4, False)import RPi.GPIO as GPIO
import time
import curses
import random
...
try:
    while True:
        ...
        if key == ord('j'):
            toggle_mode()  # 모드 변경 (수동 <-> 라인 트레이서)
            if manual_mode:
                stdscr.addstr(0, 0, "Manual Mode      ")
            else:
                stdscr.addstr(0, 0, "Line Follower Mode")
        elif key == ord('k'):
            toggle_mode()  # 모드 변경 (수동 <-> 라인 트레이서)
            if manual_mode:
                stdscr.addstr(0, 0, "Manual Mode      ")
        ...
        if manual_mode:
            ...
            # 라인 트레이서 모드
            sensor_right = GPIO.input(left_sensor)
            sensor_left = GPIO.input(right_sensor)
